...
// Definir grupos de cursos (puedes ajustarlos según necesites)
$grupos_cursos = [
    ['494','482','509','415','507','481','508','485','486','490','526','510','511'],  // Constitución
    ['428'],      // Grupo 2 (como en tu ejemplo anterior)
    
];

// Ruta para archivos temporales
$tempDir = sys_get_temp_dir();
$zipFilePath = $tempDir . '/reportes_moodle_' . date('YmdHis') . '.zip';
$zip = new ZipArchive();
...
try {
    ...
    // Array para almacenar los archivos generados
    $excelFiles = [];

    // Iterar sobre cada grupo de cursos
    foreach ($grupos_cursos as $index => $curso_ids) {
        $curso_ids_str = implode(',', $curso_ids); // Convertir IDs a string para SQL

        // Obtener nombres y conteos de actividades solo para los cursos del grupo
        $forum_sql = "
            ...
            WHERE f.course IN ($curso_ids_str) 
              AND f.name != 'Avisos' 
        ...
        $lesson_sql = "
            ...
            WHERE l.course IN ($curso_ids_str) 
        ...
        $assign_sql = "
            ...
            WHERE a.course IN ($curso_ids_str) 
        ...
        $quiz_sql = "
            ...
            WHERE q.course IN ($curso_ids_str) 
        ...
        $glossary_sql = "
            ...
            WHERE g.course IN ($curso_ids_str) 
        ...
        $scorm_sql = "
            ...
            WHERE s.course IN ($curso_ids_str) 
        ...
        $log_agg_sql = "
            ...
            WHERE log.action = 'viewed'
              AND log.timecreated BETWEEN EXTRACT(EPOCH FROM :fecha_inicio::timestamp) 
                                     AND EXTRACT(EPOCH FROM :fecha_fin::timestamp)
              AND log.courseid IN ($curso_ids_str)
        ...
        // Añadir encabezados dinámicos en el orden deseado
        for ($i = 1; $i <= $max_forums; $i++) {
        ...
        // Generar el archivo Excel temporal para este grupo
        $excelFilePath = $tempDir . '/reporte_grupo_' . ($index + 1) . '_' . date('YmdHis') . '.xlsx';
        $writer = new Xlsx($spreadsheet);
        $writer->save($excelFilePath);
        $excelFiles[] = $excelFilePath; // Guardar la ruta para el ZIP
    }
    ...
    $mail->Subject = 'Reportes de interacciones en foros Moodle (ZIP)';
    $mail->Body = 'Cordial Saludo, Adjunto los reportes de interacciones en foros de Moodle en un archivo ZIP.';
    $mail->addAttachment($zipFilePath, 'reportes_moodle.zip');
    ...
    try {
        $mail->send();
        $mail_notificacion->Subject = 'Éxito: Reportes de Moodle enviados';
        $mail_notificacion->Body = 'Los reportes de interacciones en foros de Moodle fueron enviados exitosamente a ' . implode(', ', $correo_destino) . ' el ' . date('Y-m-d H:i:s') . ' en un archivo ZIP.';
        $mail_notificacion->send();
        echo "Reportes generados y enviados correctamente en un archivo ZIP.";
    } catch (Exception $e) {
        $mail_notificacion->Subject = 'Fallo: Error al enviar reportes de Moodle';
        $mail_notificacion->Body = 'Hubo un error al enviar los reportes de interacciones en foros de Moodle: ' . $e->getMessage();
        $mail_notificacion->send();
        echo "Error al enviar los reportes: " . $e->getMessage();
    }
