...
try {
    ...
    foreach ($forum_results as $i => $row) {
        ...
        $forum_dates[$i + 1] = [
            'apertura' => $row['fecha_apertura'] ? date('Y-m-d H:i:s', $row['fecha_apertura']) : '',
            'cierre' => $row['fecha_cierre'] ? date('Y-m-d H:i:s', $row['fecha_cierre']) : ''
        ];
    }

    $lesson_sql = "SELECT DISTINCT id AS lesson_number, name AS lessonname, available AS fecha_apertura, deadline AS fecha_cierre 
                   FROM mdl_lesson WHERE course IN ('530') ORDER BY name";
    $stmt = $pdo->query($lesson_sql);
    ...
    $assign_sql = "SELECT DISTINCT id AS assign_number, name AS assignname, allowsubmissionsfromdate AS fecha_apertura, duedate AS fecha_cierre 
                   FROM mdl_assign WHERE course IN ('530') ORDER BY name";
    $stmt = $pdo->query($assign_sql);
    ...
    $quiz_sql = "SELECT DISTINCT id AS quiz_number, name AS quizname, timeopen AS fecha_apertura, timeclose AS fecha_cierre 
                 FROM mdl_quiz WHERE course IN ('530') ORDER BY name";
    $stmt = $pdo->query($quiz_sql);
    ...
    $glossary_sql = "SELECT DISTINCT id AS glossary_number, name AS glossaryname, timecreated AS fecha_apertura, timemodified AS fecha_cierre 
                     FROM mdl_glossary WHERE course IN ('530') ORDER BY name";
    $stmt = $pdo->query($glossary_sql);
    ...
    $scorm_sql = "SELECT DISTINCT id AS scorm_number, name AS scormname, timeopen AS fecha_apertura, timeclose AS fecha_cierre 
                  FROM mdl_scorm WHERE course IN ('530') ORDER BY name";
    $stmt = $pdo->query($scorm_sql);
    ...
    // Create temporary tables for pre-aggregation (data only)
    ...
    // Main query with pre-aggregated data (sin UserInfo ni CourseInfo)
    $sql = "SELECT 
    ...
    // Add dynamic forum columns (fecha_apertura, fecha_cierre, visitas, envios, nota, retroalimentada)
    ...
    if (!empty($resultados)) {
        ...
        // Base headers (reducidos a 9 columnas)
        ...
        // Add dynamic assignment headers
        for ($i = 1; $i <= $max_assigns; $i++) {
        ...
        $mail->Subject = 'Reporte de actividades Diplomado en Gestión de Proyectos - ' . $hoy->format('Y-m-d');
        $mail->Body = 'Cordial Saludo, Adjunto el Reporte de interacciones en foros, lecciones, tareas, quizzes, glosarios y SCORM de Moodle.';
        $mail->addAttachment($tempFilePath, 'reporte.xlsx');
        $mail->send();

        fclose($tempFile);
    }
    mail($correo_notificacion, 'Estado Reporte', 'El reporte de interacciones en foros, lecciones, tareas, quizzes, glosarios y SCORM fue enviado correctamente.');
} catch (Exception $e) {
