...
try {
    ...
    // Crear archivo ZIP
    $zip = new ZipArchive();
    $zip_file = $temp_dir . '/' . uniqid('reporteCEI_', true) . '.zip';
    ...
    $mail->Subject = 'Reporte de Ingresos y Mensajes Semanal del Curso Excel para la generación de informes  - ' . $fecha_para_nombre;
    $mail->Body = 'Cordial Saludo, Adjunto el Reporte de Ingresos y Mensajes Semanal del Curso Excel para la generación de informes, que contiene los archivos de estudiantes y profesores.';
    if (file_exists($zip_file)) {
        $mail->addAttachment($zip_file, "reporte_semanal_curso_excel_informes_{$fecha_para_nombre}.zip");
    }
    $mail->send();
