...
// Calcular fechas: inicio fijo el 17 de abril de 2025, fin el lunes más reciente
$fecha_inicio_fija = new DateTime('2025-04-17'); 
...
try {
    ...
    $nombre_estudiantes = "estudiantes_curso_excel_{$fecha_para_nombre}.csv";
    $nombre_profesores = "profesores_curso_excel_{$fecha_para_nombre}.csv";

    // Crear archivos temporales
    $temp_dir = sys_get_temp_dir();
    $temp_estudiantes = $temp_dir . '/' . uniqid('estudiantes_curso_excel', true) . '.csv';
    $temp_profesores = $temp_dir . '/' . uniqid('profesores_curso_excel', true) . '.csv';
    ...
    $mail->setFrom('noreply-univirtual@utp.edu.co', 'Reporte Moodle Curso Excel Generacion Informes');
    foreach ($correo_destino as $correo) {
        $mail->addAddress($correo);
    }
    $mail->Subject = 'Reporte de Ingresos y Mensajes Acumulado del Curso Excel para la generación de informes  - ' . $fecha_para_nombre;
    $mail->Body = 'Cordial Saludo, Adjunto el Reporte de Ingresos y Mensajes Acumulado del Curso Excel para la generación de informes, que contiene los archivos de estudiantes y profesores.';
    if (file_exists($zip_file)) {
        $mail->addAttachment($zip_file, "reporte_ingresos_acumulado_curso_excel_informes_{$fecha_para_nombre}.zip");
    }
    $mail->send();
    ...
    mail($correo_notificacion, 'Estado Reporte', 'El reporte del Curso Excel para la generación de informes fue enviado correctamente.');
} catch (Exception $e) {
