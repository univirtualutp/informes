...
try {
    ...
    $grupos_cursos = [
        'constitucion_politica' => ['494','482','509','415','507','481','508','485','486','490','526','510','511'],
        'Diseno_unidades' => ['419','475'],
        //'Taller_Liderazgo' => ['550', '551']
    ];
    ...
    foreach ($grupos_cursos as $nombre_grupo => $cursos) {
        ...
        // Foros y sus discusiones (solo las que contienen "Momento")
        $forum_sql = "SELECT DISTINCT f.id AS forum_id, f.name AS forumname, f.duedate AS fecha_apertura, f.cutoffdate AS fecha_cierre,
                      fd.id AS discussion_id, fd.name AS discussionname
                      FROM mdl_forum f
                      LEFT JOIN mdl_forum_discussions fd ON fd.forum = f.id AND fd.name LIKE '%Momento%'
                      WHERE f.course = :curso AND f.name != 'Avisos' ORDER BY f.name, fd.name";
        ...
        $max_forums = 0;
        $current_forum = null;
        foreach ($forum_results as $row) {
            if ($current_forum !== $row['forum_id']) {
                $max_forums++;
                $current_forum = $row['forum_id'];
                $forum_names[$max_forums] = $row['forumname'];
                $forum_dates[$max_forums] = [
        ...
        for ($i = 1; $i <= $max_forums; $i++) {
            ...
            foreach ($forum_discussions[$i] as $discussion) {
                $headers[] = '';
                // Usar el nombre de la discusión como subencabezado, reemplazando espacios por guiones bajos
                $discussion_name = str_replace(' ', '_', $discussion['name']) . '_envíos';
                $sub_headers[] = $discussion_name; // Ejemplo: Momento_1_envíos
            }
        }
        ...
        foreach ($cursos as $curso_id) {
            ...
            $forum_agg_sql = "
                ...
                SELECT 
                    ...
                    SUM(COUNT(DISTINCT fp.id)) OVER (PARTITION BY fp.userid, f.course, f.id) AS total_forum_posts,
                    COALESCE(gg.finalgrade, 0) AS nota_final,
                    ROW_NUMBER() OVER (PARTITION BY fp.userid, f.course ORDER BY f.name) AS forum_number,
                    ROW_NUMBER() OVER (PARTITION BY fp.userid, f.course, f.id ORDER BY fd.name) AS discussion_number
                FROM mdl_forum_posts fp
                JOIN mdl_forum_discussions fd ON fp.discussion = fd.id AND fd.name LIKE '%Momento%'
                ...
                WHERE fp.created BETWEEN EXTRACT(EPOCH FROM :fecha_inicio::timestamp) 
                  ...
                  AND f.name != 'Avisos'
                  AND f.course = :curso
                GROUP BY fp.userid, f.course, f.id, f.name, fd.id, fd.name, gg.finalgrade";
            ...
            for ($i = 1; $i <= $max_forums; $i++) {
                ...
                MAX(CASE WHEN fi.forum_number = $i THEN GREATEST(COALESCE(flog.num_views, 0), COALESCE(fi.total_forum_posts, 0)) END) AS visitas_forum_$i,
                MAX(CASE WHEN fi.forum_number = $i THEN fi.nota_final END) AS nota_final_forum_$i,";
                foreach ($forum_discussions[$i] as $disc_idx => $discussion) {
                    $disc_num = $disc_idx + 1;
                    $discussion_name = str_replace(' ', '_', $discussion['name']) . '_envíos';
                    $sql .= "
                    MAX(CASE WHEN fi.forum_number = $i AND fi.discussion_number = $disc_num THEN fi.num_posts END) AS \"{$discussion_name}\",";
                }
            }
    ...
    $mail->Body = "Cordial Saludo, Adjunto el reporte de actividades pregrado en un archivo ZIP que contiene los reportes de todos los grupos de cursos.";
